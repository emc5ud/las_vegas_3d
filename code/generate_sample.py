...
FMT_TEST_HEIGHT_IMAGE_PATH = str(
    Path("{datapath:s}/") /
    Path("dsm/dsm_{image_id:s}.tif"))
...
FMT_MUL_BANDCUT_TH_PATH = PREPROC_IMAGE_DIR + "/mul_bandcut{}.csv"
...
def get_8chan_image(image_id, datapath, bandstats, is_test=False):
...
def get_mask_image(df, image_id):
...
def get_height_image(image_id, datapath, bandstats, is_test=False):
    fn = get_train_height_path_from_imageid(
        image_id, datapath)
    if is_test:
        fn = get_test_height_path_from_imageid(
            image_id, datapath)

    with rasterio.open(fn, 'r') as f:
        values = f.read().astype(np.float32)
        #sets the minimum to 0, max unbounded
        for chan_i in range(1):
            values[chan_i] = np.clip(values[chan_i], a_min=0, a_max=None)
    values = np.swapaxes(values, 0, 2)
    values = np.swapaxes(values, 0, 1)
    assert values.shape == (650, 650, 1)
    return values

def prep_image_height(area_id, datapath, is_valtrain=True):
    prefix = area_id_to_prefix(area_id)
    bandstats_height = __load_mul_bandstats(area_id)

    logger.info("prep_height for {}".format(prefix))
    if is_valtrain:
    ...
    logger.info("Prepare height container: {}".format(fn_height))
    with tb.open_file(fn_height, 'w') as f:
        for image_id in tqdm.tqdm(df.index, total=len(df)):
            im_height = get_height_image(image_id, datapath, bandstats_height)
            assert(im_height.shape == (650,650,1))
            if np.percentile
            atom = tb.Atom.from_dtype(im_height.dtype)
            filters = tb.Filters(complib='blosc', complevel=9)
            ds = f.create_carray(f.root, image_id, atom,
                                 im_height.shape,
                                 filters=filters)
            ds[:] = im_height
...
def prep_image_mask(area_id, is_valtrain=True):
    ...
    with tb.open_file(fn_mask, 'w') as f:
        for image_id in tqdm.tqdm(df.index, total=len(df)):
            im_mask = get_mask_image(df_summary, image_id)
...
def prep_mul_image_store_train(area_id, datapath, is_valtrain=True):
    ...
    with tb.open_file(fn_store, 'w') as f:
        for image_id in tqdm.tqdm(df_list.index, total=len(df_list)):
            image_mul = get_8chan_image(image_id, datapath, bandstats_mul)
            atom = tb.Atom.from_dtype(image_mul.dtype)
            filters = tb.Filters(complib='blosc', complevel=9)
            ds = f.create_carray(f.root, image_id, atom,
                                 image_mul.shape,
                                 filters=filters)
...
def get_test_height_path_from_imageid(image_id, datapath):
    prefix = image_id_to_prefix(image_id)
    return FMT_TEST_HEIGHT_IMAGE_PATH.format(
        datapath=datapath, image_id=image_id)
...
def prep_mulmean(area_id, datapath):
    ...
    with tb.open_file(fn_im, 'r') as f:
        for idx, image_id in enumerate(image_list):
            ...
            assert (im.shape == (650, 650, 8))
            X_train.append(im)


    X_mean = np.array(X_train).mean(axis=0)
    assert(X_mean.shape == (650, 650,8))
...
def preproc_train(datapath):
    ...
    # Heights (Target output)
    if Path(FMT_VALTRAIN_HEIGHT_STORE.format(prefix)).exists():
        logger.info("Generate HEIGHT (valtrain) ... skip")
    else:
        logger.info("Generate HEIGHT (valtrain)")
        prep_image_height(area_id, datapath, is_valtrain=True)
    if Path(FMT_VALTEST_HEIGHT_STORE.format(prefix)).exists():
        logger.info("Generate HEIGHT (valtest) ... skip")
    else:
        logger.info("Generate HEIGHT (valtest)")
        prep_image_height(area_id, datapath, is_valtrain=False)
