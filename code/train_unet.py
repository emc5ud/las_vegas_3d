...
* Input: generate sample

Author: Kohei <i@ho.lc>

* slight modifications by Eric Culbertson
"""
...
MODEL_NAME = 'multipan_height_footprint'
ORIGINAL_SIZE = 650
INPUT_SIZE = 256
STRIDE_SZ = 197
total_time = 0
...
IMAGE_DIR = "/data/working/images/{}".format('sample')
PREPROC_IMAGE_DIR = "/data/working/images/{}".format('preproc')
...
FMT_VALTRAIN_IMAGELIST_PATH = PREPROC_IMAGE_DIR + "/{prefix:s}_valtrain_ImageId.csv"
FMT_VALTEST_IMAGELIST_PATH = PREPROC_IMAGE_DIR + "/{prefix:s}_valtest_ImageId.csv"
...
FMT_VALTRAIN_HEIGHT_STORE = IMAGE_DIR + "/valtrain_{}_height.h5"
FMT_VALTEST_HEIGHT_STORE = IMAGE_DIR + "/valtest_{}_height.h5"

FMT_TRAIN_IMAGELIST_PATH = PREPROC_IMAGE_DIR + "/{prefix:s}_train_ImageId.csv"
FMT_TEST_IMAGELIST_PATH = PREPROC_IMAGE_DIR + "/{prefix:s}_test_ImageId.csv"
...
FMT_TRAIN_HEIGHT_STORE = IMAGE_DIR + "/train_{}_height.h5"
FMT_TEST_HEIGHT_STORE = IMAGE_DIR + "/test_{}_height.h5"
...
def _internal_test(area_id):
    ...
    with open(fn_out, 'w') as f:
        ...
        for idx, image_id in enumerate(df_test.index.tolist()):
            ...
            df_poly = mask_to_poly(pred_values, image_id, prefix, min_polygon_area_th=min_th)
            if len(df_poly) > 0:
...
def _internal_validate_fscore_wo_pred_file(area_id,
    ...
    with open(fn_out, 'w') as f:
        f.write("ImageId,BuildingId,PolygonWKT_Pix,PolygonWKT_Geo,Confidence\n")
...
def mask_to_poly(mask, image_id, prefix, min_polygon_area_th=MIN_POLYGON_AREA):
    ...
    df.loc[:, 'wkt_geo'] = df.poly.apply(lambda x: shapely.wkt.dumps(
        georeference_pixel_point_train(x, image_id, prefix), rounding_precision=15)
    )
...
def georeference_pixel_point_train(point, image_id, prefix):
    fn_im = "/data/train/" + prefix + "_Train" + "/Pan/Pan_" + prefix + "_" + image_id
    x = point.x
    y = point.y
    src = gdal.Open(fn_im)
    ...
    while 1:
        ...
        with tb.open_file(fn_im, 'r') as f_im:
            for i_batch in range(n_batch):
                ...
                y_test = y_test.reshape((-1, INPUT_SIZE, INPUT_SIZE, 1))
    ...
    if enable_tqdm:
        ...
        while 1:
            ...
            with tb.open_file(fn_im, 'r') as f_im, \
                ...
                tb.open_file(fn_height, 'r') as f_height:
                for i_batch in range(n_batch):
                    target_slice_ids = slice_id_list[
                                   i_batch*batch_size:(i_batch+1)*batch_size
                                   ]
                    ...
                    for slice_id in target_slice_ids:
                        ...
                        y_train.append(mask)
                        height = np.array(f_height.get_node('/' + slice_id))
                        h_train.append(height)
                    X_train = np.array(X_train)
                    y_train = np.array(y_train)
                    h_train = np.array(h_train)
                    y_train = y_train.reshape((-1, INPUT_SIZE, INPUT_SIZE, 1))
                    h_train = h_train.reshape((-1, INPUT_SIZE, INPUT_SIZE, 1))
                    if immean is not None:
                        X_train = X_train - immean

                    yield (X_train, [y_train, h_train])
...
def generate_valtrain_batch(area_id, batch_size=8, immean=None):
    ...
    fn_height = FMT_VALTRAIN_HEIGHT_STORE.format(prefix)
...
def get_unet():
    ...
    merge_params = dict(mode='concat', concat_axis=3)
    ...
    up6 = merge([UpSampling2D(size=(2, 2))(conv5), conv4], **merge_params)
    conv6 = Convolution2D(256, 3, 3, **conv_params)(up6)
    conv6 = Convolution2D(256, 3, 3, **conv_params)(conv6)

    up7 = merge([UpSampling2D(size=(2, 2))(conv6), conv3], **merge_params)
    conv7 = Convolution2D(128, 3, 3, **conv_params)(up7)
    conv7 = Convolution2D(128, 3, 3, **conv_params)(conv7)

    up8 = merge([UpSampling2D(size=(2, 2))(conv7), conv2], **merge_params)
    conv8 = Convolution2D(64, 3, 3, **conv_params)(up8)
    conv8 = Convolution2D(64, 3, 3, **conv_params)(conv8)

    up9 = merge([UpSampling2D(size=(2, 2))(conv8), conv1], **merge_params)
    conv9 = Convolution2D(32, 3, 3, **conv_params)(up9)
    conv9 = Convolution2D(32, 3, 3, **conv_params)(conv9)

    conv10 = Convolution2D(1, 1, 1, activation='sigmoid', name='building_pred')(conv9)
    conv11 = Convolution2D(1, 1, 1, name='height_pred')(conv9)

    model = Model(input=inputs, output=[conv10, conv11])

    model.compile(optimizer=Adam(), loss=['binary_crossentropy', 'logcosh'],
                  metrics={'building_pred': [jaccard_coef,jaccard_coef_int, 'accuracy'],
                           'height_pred': 'mse'}, loss_weights=[.7, .3])
...
def get_mul_mean_image(area_id):
    ...
    with tb.open_file(FMT_MULMEAN.format(prefix), 'r') as f:
        ...
        im_mean = np.swapaxes(im_mean, 0, 2)
        im_mean = np.swapaxes(im_mean, 1, 0)
    return im_mean
...
def get_valtest_data(area_id):
    ...
    h_val = []
    fn_mask = FMT_VALTEST_MASK_STORE.format(prefix)
    fn_height = FMT_VALTEST_HEIGHT_STORE.format(prefix)
    with tb.open_file(fn_mask, 'r') as f, tb.open_file(fn_height, 'r') as f1:
        ...
        for idx, image_id in enumerate(df_test.ImageId.tolist()):
            for slice_pos in range(9):
                ...
                mask = np.array(f.get_node('/' + slice_id))
                mask = (mask > 0.5).astype(np.uint8)
                height=  np.array(f1.get_node('/' + slice_id))
                y_val.append(mask)
                h_val.append(height)
    y_val = np.array(y_val)
    h_val = np.array(h_val)
    y_val = y_val.reshape((-1, INPUT_SIZE, INPUT_SIZE, 1))
    h_val = h_val.reshape((-1, INPUT_SIZE, INPUT_SIZE, 1))
    print(y_val.shape)
    return X_val, [y_val, h_val]
    ...
    with tb.open_file(fn_mask, 'r') as f, tb.open_file(fn_height, 'r') as f1:
        for idx, image_id in enumerate(df_train.ImageId.tolist()):
            for slice_pos in range(9):
                ...
                mask = np.array(f.get_node('/' + slice_id))
                mask = (mask > 0.5).astype(np.uint8)
                height=  np.array(f.get_node('/' + slice_id))
                y_val.append(mask)
                h_val.append(height)
    y_val = np.array(y_val)
    h_val = np.array(h_val)
...
def validate(datapath):
    ...
    start_time = timeit.default_timer()
    model.fit_generator(
        generate_valtrain_batch(area_id, batch_size=8, immean=None),
        samples_per_epoch=len(df_train) * 9,
        nb_epoch=1,
        verbose=1,
        validation_data=(X_val, y_val),
        callbacks=[model_checkpoint, model_earlystop, model_history])
    logger.info("" + str(timeit.default_timer() - start_time) + " elapsed time for one epoch")
    logger.info("" + str(total_time) + " elapsed time in generator")

# model.fit_generator(
    #     generate_valtrain_batch(area_id, batch_size=2),
    #     samples_per_epoch=len(df_train) * 9,
    #     nb_epoch=20,
    #     verbose=1,
    # )
    model.save(FMT_VALMODEL_LAST_PATH.format(prefix))
    #with open("/data/working/models/multipan_augments/unet.json", "w") as fjson:
    #    fjson.write(model.to_json())

    #model.save_weights("/data/working/models/multipan_augments/unet_weights.h5")
