...
def generate_valtest_sample(area_id,
                           sample_size=9,
                           immean=None,
                           enable_tqdm=False):
    prefix = area_id_to_prefix(area_id)
    ...
    with tb.open_file(fn_im, 'r') as f_im, \
            ...
            target_slice_ids = slice_id_list[0:sample_size]
            ...
            for slice_id in target_slice_ids:
                ...
                print(im.shape)
                imsave('/tmp/'+ slice_id + ".tif", im)
                X_train.append(im)
                im = np.swapaxes(im, 0, 1)
                im = np.swapaxes(im, 1, 2)
                mask = np.array(f_mask.get_node('/' + slice_id))
                mask = (mask > 0).astype(np.uint8)
                imsave('/tmp/'+ slice_id + "_mask.tif", mask)
                y_train.append(mask)
                height = np.array(f_height.get_node('/' + slice_id))
                h_train.append(height)
                print("height max: ", np.max(height))
                imsave('/tmp/'+ slice_id + "_height.tif", height)
                im = im[None, ...]
                mask = mask[None, ...]
                height = height[None, ...]
                yield (im, [mask, height], slice_id)


@cli.command()
@click.argument('datapath', type=str)
def eval_smallsample(datapath):
    with tf.device('/GPU:0'):
        area_id = directory_name_to_area_id(datapath)
        prefix = area_id_to_prefix(area_id)
        logger.info("")
        model = load_model("/data/working/models/multipan_height_footprint/AOI_2_Vegas_val_weights_last.h5",
        ...
        sess = tf.Session(config=config)
        for sample in generate_valtest_sample(area_id):
            (X, [M, H], slice_id) = sample
            i+=1
            print(slice_id)
            [M_pred, H_pred] = model.predict(X)
            imsave('/tmp/'+ slice_id + "_mask_pred.tif", M_pred)
            imsave('/tmp/'+ slice_id + "_height_pred.tif", H_pred)
...
def evalfscore(datapath):
    ...
    y_pred = unet._internal_validate_predict_best_param(
        area_id, enable_tqdm=True)

    # no ensemble so no average
    # logger.info("Averaging")
